module WktimeHelper 
    ...
    export = FCSV.generate(:col_sep => l(:general_csv_separator)) do |csv|
      ...
      csv << headers.collect {|c| Redmine::CodesetUtil.from_utf8(
		...
		weeklyHash.each do |key, matrix|
			matrix_values, j = getColumnValues(matrix, totals, unitLabel,0)
  ...
  def wktime_to_pdf(entries, user, startday, unitLabel)
	...
	if !unitLabel.blank?
		...
		col_width[0] = (table_width - (8*14))*0.20
		col_width[1] = (table_width - (8*14))*0.45
		col_width[2] = (table_width - (8*14))*0.15
		col_width[3] = (table_width - (8*14))*0.20
		col_id_width  = 14
	...
	weeklyHash.each do |key, matrix|
		matrix_values, j = getColumnValues(matrix, totals, unitLabel, j)
		col_values = matrix_values[0]
		base_x = pdf.GetX
		base_y = pdf.GetY
...
def getColumnValues(matrix, totals, unitLabel, j=0)
	col_values = []
	matrix_values = []
	unless matrix.blank?
		matrix.each do |rows|
...
def getTotalValues(totals, hoursIndex,unit)
	...
	def sendNonSubmissionMail
		...
		diff = working_days(startDate,deadline)
		diff = diff + 1
