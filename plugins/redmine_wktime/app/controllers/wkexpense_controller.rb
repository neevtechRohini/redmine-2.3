...
private
  ...
  def findBySql(selectStr,sqlStr,wkSelectStr,wkSqlStr)
	...
	@entries = WkExpenseEntry.find_by_sql(wkSelectStr + " ,exp.currency" + sqlStr + 
		" inner join wk_expense_entries exp on v1.id = exp.id " + wkSqlStr +
  ...
  def setValueForSpField(timeEntry,spValue,decimal_separator,entry)
	timeEntry.amount = spValue.blank? ? nil : spValue.gsub(decimal_separator, '.').to_f	
	timeEntry.currency = getUnit(entry)  	
  end
