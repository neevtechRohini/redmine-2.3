...
helper :custom_fields
  ...
  def index
    ...
    respond_to do |format|
      format.html {
		...
		sqlStr += " (select " + getDateSqlString('t.spent_on') + " as startday, "	
  ...
  def update
  	respond_to do |format|
		format.html {
			...
			errorMsg = gatherWkCustomFields(@wktime) if @wkvalidEntry
			...
			@wktime.transaction do
				begin				
					if errorMsg.blank? && (!params[:wktime_save].blank? ||
						(!params[:wktime_submit].blank? && @wkvalidEntry))
						@wktime.status = :n
						# save each entry
				...
				if errorMsg.nil?
					...
					flash[:error] = l(:error_te_save_failed, :label => setEntityLabel, :error => errorMsg)
  ...
  def getissues
	...
	if Setting.plugin_redmine_wktime['wktime_closed_issue_ind'].to_i == 1
		if !params[:tracker_id].blank? && params[:tracker_id] != ["0"]
			issues = Issue.find_all_by_project_id(params[:project_id] || params[:project_ids] , 
			:conditions =>  ["#{Issue.table_name}.tracker_id in ( ?) ", params[:tracker_id]] , :order => 'project_id')	

		elsif !params[:term].blank? 
			  	if subjectPart.present?
					if subjectPart.match(/^\d+$/)						
						cond = ["LOWER(#{Issue.table_name}.subject) LIKE ? OR #{Issue.table_name}.id=?", "%#{subjectPart.downcase}%","#{subjectPart.to_i}"]
						else
						cond = ["LOWER(#{Issue.table_name}.subject) LIKE ?", "%#{subjectPart.downcase}%"]
					end
		...
		if !params[:tracker_id].blank? && params[:tracker_id] != ["0"]			
			cond = ["(#{IssueStatus.table_name}.is_closed = ? OR #{Issue.table_name}.updated_on >= ?) AND  #{Issue.table_name}.tracker_id in ( ?)", false, @startday,params[:tracker_id]]			
		elsif !params[:term].blank? 
			if subjectPart.present?
				if subjectPart.match(/^\d+$/)					
					cond = ["(LOWER(#{Issue.table_name}.subject) LIKE ? OR #{Issue.table_name}.id=?)  AND #{IssueStatus.table_name}.is_closed = ? ", "%#{subjectPart.downcase}%","#{subjectPart.to_i}",false]
				else
					cond = ["LOWER(#{Issue.table_name}.subject) LIKE ?  AND #{IssueStatus.table_name}.is_closed = ? ", "%#{subjectPart.downcase}%",false]
				end				
			 end  
		else
			cond =["#{IssueStatus.table_name}.is_closed = ? OR #{Issue.table_name}.updated_on >= ?", false, @startday]
		end	
					...
					ids.each_with_index do |id, k|
						if disabled[k] == "false"
							if(!id.blank? || !hours[j].blank?)
								...
								setValueForSpField(timeEntry,hours[j],decimal_separator,entry)
...
def deleteWktime
  ...
  def retrieve_date_range
	...
	def set_visible_issues(entry)
        ...
        if @projectIssues[project_id].blank?
            ...
            if Setting.plugin_redmine_wktime['wktime_closed_issue_ind'].to_i == 1                
                ...
                if !Setting.plugin_redmine_wktime[getTFSettingName()].blank? &&  Setting.plugin_redmine_wktime[getTFSettingName()] != ["0"]
                    cond = ["(#{IssueStatus.table_name}.is_closed = ? OR #{Issue.table_name}.updated_on >= ?) AND  #{Issue.table_name}.tracker_id in ( ?) ",false, @startday,Setting.plugin_redmine_wktime[getTFSettingName()]]
                else
                    cond =["#{IssueStatus.table_name}.is_closed = ? OR #{Issue.table_name}.updated_on >= ?",false, @startday]
                end
                allIssues = Issue.find_all_by_project_id(project_id,
	...
	def setValueForSpField(timeEntry,spValue,decimal_separator,entry)
		timeEntry.hours = spValue.blank? ? nil : spValue.gsub(decimal_separator, '.').to_f
	end
