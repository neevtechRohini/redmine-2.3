...
function projectChanged(projDropdown, row){
	...
	{	
		...
		$.ajax({
			url: issUrl,
			type: 'get',
			data: {project_id: id, user_id: uid,tracker_id: trackerListArr, format:fmt,startday:startday},
...
function trackerFilterChanged(trackerList){
	if(trackerList!=null){	
		...
		$.ajax({
			url: issUrl,
			type: 'get',
			data: {project_ids:projectids,user_id: uid, tracker_id: trackerListArr, format:fmt,startday:startday},			
...
function validateMinhour(minHour,nonWorkingDay){
	...
	if(valid && totalhr > 0 && submissionack!=''){
