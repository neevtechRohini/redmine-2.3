...
Redmine::Plugin.register :redmine_wktime do
  ...
  menu :top_menu, :wkTime, { :controller => 'wktime', :action => 'index' }, :caption => :label_te, :if => Proc.new { User.current.logged? } 	
...
class WktimeHook < Redmine::Hook::ViewListener
	...
	def view_projects_show_sidebar_bottom(context={})		
		if !context[:project].blank?
			...
			if !project_ids.blank? && (project_ids == [""] || project_ids.include?("#{context[:project].id}")) && User.current.allowed_to?(:view_time_entries, context[:project])
